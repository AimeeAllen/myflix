...
describe QueueItemsController do
  describe "GET index" do
    context "authenticated user" do
      ...
      it "sets @queue_items" do
        queue = []
        3.times {queue << Fabricate(:queue_item, user: @user)}
        get :index
  ...
  describe "POST create" do
    context "authenticated user" do
      ...
      it "assigns an order to be the last number in the queue" do
        ...
        post :create, video_id: video.id
        expect(QueueItem.last.order).to eq(@user.queue_items.size)
      end
      it "doesn't add the video to the queue if it is already in the queue" do
        Fabricate(:queue_item, user: @user, video: video)
        post :create, video_id: video.id
        expect(QueueItem.where(user_id: @user.id, video_id: video.id).size).to eq(1)
      end
  ...
  describe "POST update_queue" do
    context "logged in user" do
      before {log_in_a_user}
      let(:queue_item1) {Fabricate(:queue_item, user: @user, order: 1)}
      let(:queue_item2) {Fabricate(:queue_item, user: @user, order: 2)}
    ...
    it_behaves_like "no logged in user" do
      let(:action) {post "update_queue", queue_items: [{id: 34, order: 3}, {id: 12, order: 1}]}
    end
  end
end
